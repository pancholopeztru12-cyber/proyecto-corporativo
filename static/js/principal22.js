...
// Registros de prueba específicos para la pantalla 2.2
const registrosDemo = [
    { nombre: "Dato Final 2.2 - X", descripcion: "Último módulo del sistema principal", estado: "Activo" },
    { nombre: "Dato Final 2.2 - Y", descripcion: "Validación de permisos de seguridad", estado: "Activo" },
    { nombre: "Dato Final 2.2 - Z", descripcion: "Registro deshabilitado por el administrador", estado: "Inactivo" }
];
...
// Dibuja la tabla dinámica y evalúa qué botones pintar según los permisos
        ...
        if (response.ok) {
            ...
            if (menuPrincipal2.length > 0) {
                ...
                menuPrincipal2.forEach(nombre => {
                    const link = `${nombre.toLowerCase().replace(/\s+/g, '')}.html`;
                    // Resaltar la página actual (Principal 2.2)
                    const style = (nombre === 'Principal 2.2') ? 'color: #4f46e5; font-weight: bold; text-decoration: none;' : 'color: #cbd5e1; text-decoration: none;';
